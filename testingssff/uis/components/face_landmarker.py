...
from testingssff import face_landmarker, state_manager, wording
from testingssff.common_helper import calc_float_step
from testingssff.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = testingssff.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(testingssff.choices.face_landmarker_score_range),
		minimum = testingssff.choices.face_landmarker_score_range[0],
		maximum = testingssff.choices.face_landmarker_score_range[-1]
	)
